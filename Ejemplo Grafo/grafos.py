...
class Grafo:
    ...
    def agregar_arista(self, u, v, peso=1):
        ...
        if not self.es_dirigido:
            self.grafo[v].add(u)  # Corrected from self.grado[v].add(u) to self.grafo[v].add(u)
            print(f"Arista {v} -> {u} (bidireccional) agregada.")

    def obtener_vecinos(self, vertice):
        if vertice in self.grafo:  # Corrected from self.grado to self.grafo
    ...
    def es_conexo(self):
        ...
        return len(recorrido_bfs) == len(self.grafo) # Added logic for es_conexo
    ...
    def encontrar_camino(self, inicio, fin):
        if inicio not in self.grafo or fin not in self.grafo:
            ...
            return [] # Changed from return to return [] for consistency
...
print(f"Camino dirigido de Fin a Inicio: {camino_dirigido_no_existente}") # Removed stray ")"